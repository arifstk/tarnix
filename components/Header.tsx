...
//   const isUser = (!!session?.user as any)?.role === "user";
    ...
    <header className="sticky top-0 z-50 backdrop-blur-md shadow-[0_1px_30px_rgba(0,0,0,0.4)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center md:hidden gap-3">
          <MobileNav role={role} />   {/* ← hamburger is md:hidden */}
          <Link href="/">
            <h2 className="text-2xl text-tarnix_dark_green font-black tracking-wider hover:text-tarnix_light_green hoverEffect group font-sans">
            Tarnix
          </h2>
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-52 rounded-xl bg-[#111827] border border-white/8 shadow-2xl overflow-hidden z-50">
                  ...
                  <div className="p-1.5 space-y-0.5">
                    ...
                    {role === "deliveryBoy" && (
                      <Link href="/delivery" className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-slate-300 hover:text-white hover:bg-white/[0.07] transition-all duration-150" onClick={() => setDropdownOpen(false)}>
                        My Deliveries
                      </Link>
                    )}
                  </div>
