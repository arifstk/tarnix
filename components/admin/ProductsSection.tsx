...
// // Self-contained product management panel — drop into AdminDashboard as <ProductsSection />
...
//     if (formTarget === "new") {
//       dispatch(addProduct(data as ProductInput)).then(() => setFormTarget(null));
//     } else if (formTarget) {
//       dispatch(updateProduct({ id: formTarget._id, data })).then(() => setFormTarget(null));
//     }
//   };

//   const handleQuickSave = (stock: number, discountRate: number) => {
//     if (!quickEdit) return;
//     dispatch(patchProductInventory({ id: quickEdit._id, stock, discountRate }))
//       .then(() => setQuickEdit(null));
//   };

//   const handleDelete = () => {
//     if (!confirmDel) return;
//     dispatch(deleteProduct(confirmDel._id)).then(() => setConfirmDel(null));
//   };
...
// components/admin/ProductsSection.tsx
// FIX summary (search for "FIX" comments):
//   1. handleFormSubmit: await the dispatch so errors surface correctly
//   2. ProductFormModal initial state: category was only handling string case,
//      but API returns populated object { _id, name, slug } — extract ._id
//   3. handleDelete: use .unwrap() to catch errors
...
// ─── Stock / Discount quick-edit modal ────────────────────────
...
// FIX: helper — safely extract the category _id whether it's a string or populated object
function resolveCategoryId(cat: IProduct["category"]): string {
...
const ProductFormModal = ({
  ...
  const [form, setForm] = useState<FormData>(() =>
    product
      ? {
          ...
          stock:        String(product.stock),
          // FIX: use helper — previously only handled string case, broke when category
          // came back as a populated object { _id, name, slug } from GET /api/products
  ...
  const handleSubmit = () => {
    ...
    // FIX: only require image when adding a new product (not editing)
    if (!product && !form.imageBase64) return setFormError("Please upload a product image.");
    ...
    // Only include imageBase64 when a new image was chosen (avoids re-uploading on text-only edits)
    if (form.imageBase64) payload.imageBase64 = form.imageBase64;
    onSubmit(payload);
  };
    ...
    <Modal title={product ? `Edit — ${product.name}` : "Add New Product"} onClose={onClose}>
      <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
        ...
        <div className="grid grid-cols-2 gap-3">
          <SelectInput label="Category *" value={form.category}
            onChange={e => set("category", e.target.value)}>
            {/* FIX: add a placeholder option so an empty category is visible */}
...
export default function ProductsSection() {
  const dispatch = useDispatch<AppDispatch>();
  const { items: products, loading, saving, error } = useSelector(
    (s: RootState) => s.products,
  );
  const categories = useSelector(
    (s: RootState) => (s as any).categories?.items ?? [],
  ) as { _id: string; name: string }[];

  const [search, setSearch]           = useState("");
  const [statusFilter, setStatusFilter] = useState<"all" | "active" | "draft">("all");
  const [formTarget, setFormTarget]   = useState<IProduct | null | "new">(null);
  const [quickEdit, setQuickEdit]     = useState<IProduct | null>(null);
  const [confirmDel, setConfirmDel]   = useState<IProduct | null>(null);
  ...
  // FIX: use .unwrap() so errors from rejectWithValue surface (without it the
  // promise always resolves and the modal closes even on failure)
  const handleFormSubmit = async (data: Partial<ProductInput>) => {
    try {
      if (formTarget === "new") {
        await dispatch(addProduct(data as ProductInput)).unwrap();
      } else if (formTarget) {
        await dispatch(updateProduct({ id: formTarget._id, data })).unwrap();
      }
      setFormTarget(null);
    } catch {
      // error is already in Redux state — the error banner in the section will show it
    }
  };

  const handleQuickSave = async (stock: number, discountRate: number) => {
    if (!quickEdit) return;
    try {
      await dispatch(patchProductInventory({ id: quickEdit._id, stock, discountRate })).unwrap();
      setQuickEdit(null);
    } catch { /* error shown in banner */ }
  };

  // FIX: use .unwrap() for consistent error handling
  const handleDelete = async () => {
    if (!confirmDel) return;
    try {
      await dispatch(deleteProduct(confirmDel._id)).unwrap();
      setConfirmDel(null);
    } catch { /* error shown in banner */ }
  };
  ...
  return (
    <div className="max-w-6xl space-y-5">
      ...
      {!loading && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(p => {
            // FIX: resolve category name safely for both string and populated object
            const catName = typeof p.category === "string" ? p.category : p.category.name;
            return (
              <Card key={p._id} className="relative group flex flex-col">
                <div className="flex items-start justify-between mb-3">
