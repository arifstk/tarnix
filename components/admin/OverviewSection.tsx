...
export default function OverviewSection() {
  ...
  return (
    <div className="space-y-6 text-slate-800">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {STAT_CARDS.map((s) => (
          <Card key={s.label} className={`shadow-sm ${s.glow} relative overflow-hidden group`}>
            <div className={`w-10 h-10 rounded-xl bg-linear-to-br ${s.accent} flex items-center justify-center text-lg mb-4 shadow-md text-white`}>
              {s.icon}
            </div>
            {loading ? (
              <>
              ...
              <>
                ...
                <p className={`text-xs font-semibold mt-2 ${s.positive ? "text-emerald-600" : "text-rose-600"}`}>
                  {s.positive ? "↑" : "↓"} {s.sub}
                </p>
              </>
            )}
            <div className={`absolute -bottom-5 -right-5 w-24 h-24 rounded-full bg-linear-to-br ${s.accent} opacity-5 blur-xl group-hover:opacity-10 transition-opacity duration-300`} />
          </Card>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <div className="mb-4">
            <h2 className="text-sm font-bold text-slate-900">Revenue (6 months)</h2>
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={monthlyRevenue} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                <Tooltip content={<CustomLineTooltip />} />
                <Line
                  type="monotone" dataKey="revenue" stroke="#4f46e5" strokeWidth={2.5}
                  dot={{ fill: "#4f46e5", r: 4 }} activeDot={{ r: 6, fill: "#6366f1" }}
                />
              </LineChart>
            </ResponsiveContainer>
          )}
        ...
        <Card>
          <div className="mb-4">
            <h2 className="text-sm font-bold text-slate-900">Orders This Week</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              {loading ? "Loading…" : `${weeklyOrders.reduce((s, d) => s + d.orders, 0)} total this week`}
            </p>
          </div>
              ...
              <BarChart data={weeklyOrders} barSize={24} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="orders" radius={[6, 6, 0, 0]}>
                  {weeklyOrders.map((entry, i) => (
                    <Cell
                      key={i}
                      fill={entry.orders === maxOrders ? "#4f46e5" : "rgba(99,102,241,0.25)"}
                    />
                  ))}
                </Bar>
              </BarChart>
      ...
      {/* ── Order Status Breakdown ── */}
      <Card>
        <h2 className="text-sm font-bold text-slate-900 mb-4">Order Status Breakdown</h2>
        {loading ? (
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-16" />)}
          </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {statusBreakdown.map((s) => (
              <div
                key={s.status}
                className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border ${orderColors[s.status] || "bg-slate-100 text-slate-500 border-slate-200"}`}
              >
                <span className="text-xl">{statusIcons[s.status] || "•"}</span>
                <span className="text-lg font-black">{s.count}</span>
                <span className="text-[10px] font-semibold capitalize text-center leading-tight">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Low Stock Alert */}
        <Card>
          <h2 className="text-sm font-bold text-slate-900 mb-1">Low Stock Alert</h2>
          <p className="text-xs text-slate-400 mb-4">Products with less than 10 units</p>
          {loading ? (
            <div className="space-y-3">
              {Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-12" />)}
            </div>
            ...
            <div className="text-center py-8">
              <div className="text-3xl mb-2">✅</div>
              <p className="text-sm text-emerald-600 font-semibold">All products well stocked!</p>
            </div>
          ) : (
            <div className="space-y-2">
              {lowStock.map((p) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg overflow-hidden bg-slate-100 shrink-0 flex items-center justify-center text-lg border border-slate-200">
                      {p.images?.[0]?.url ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={p.images[0].url} alt={p.name} className="w-full h-full object-cover" />
                      ) : "📦"}
                    </div>
                    <p className="text-xs font-semibold text-slate-800 truncate max-w-40">{p.name}</p>
                  </div>
                  <span className={`text-xs font-black px-2.5 py-1 rounded-full ${p.stock === 0
