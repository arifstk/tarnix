...
const roleColors: Record<Role, string> = {
  ...
  deliveryBoy: "bg-amber-50 text-amber-700 border border-amber-200",
...
const Modal = ({
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/40 backdrop-blur-xs">
    <div
      className="w-full max-w-md rounded-2xl p-6 shadow-xl bg-white border border-slate-200"
    >
      <div className="flex items-center justify-between mb-5">
...
export default function UsersSection() {
  ...
  return (
    <div className="max-w-6xl space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
        {[
          { label: "All Users", count: totalUsers, color: "bg-slate-50 hover:bg-slate-100 text-slate-700 border-slate-200", key: "all" },
          { label: "Customers", count: userCount, color: "bg-sky-50 hover:bg-sky-100 text-sky-700 border-sky-200", key: "user" },
          { label: "Admins", count: adminCount, color: "bg-violet-50 hover:bg-violet-100 text-violet-700 border-violet-200", key: "admin" },
          { label: "Delivery Boys", count: deliveryBoyCount, color: "bg-amber-50 hover:bg-amber-100 text-amber-700 border-amber-200", key: "deliveryBoy" },
        ].map((chip) => (
          <button
            key={chip.key}
            onClick={() => setRoleFilter(chip.key as Role | "all")}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-xs font-semibold transition-all ${chip.color} ${roleFilter === chip.key ? "ring-2 ring-indigo-600/20 border-indigo-500" : "opacity-80 hover:opacity-100"
              }`}
          >
            {chip.label}
            <span className="font-black">{loading ? "…" : chip.count}</span>
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {search && (
          <button
            onClick={() => setSearch("")}
            className="px-3 py-2 rounded-xl bg-slate-100 text-slate-600 hover:text-slate-900 text-sm border border-slate-200 transition-all"
          >
            Clear ✕
          </button>
        )}
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <select
                        ...
                        disabled={updatingId === u._id}
                        onChange={(e) => handleRoleChange(u._id, e.target.value as Role)}
                        className={`text-xs font-semibold px-2.5 py-1 rounded-full border-0 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 cursor-pointer transition-all bg-transparent disabled:opacity-50 ${roleColors[u.role]}`}
                      >
                        ...
                        <option value="deliveryBoy" className="bg-white text-slate-900">deliveryBoy</option>
                      </select>
                    </td>

                    {/* Orders */}
                    <td className="px-5 py-3.5">
                      <span className="text-sm font-black text-slate-900">{u.orderCount}</span>
                      <span className="text-xs text-slate-500 ml-1">orders</span>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => { setEditModal(u); setEditRole(u.role); }}
                          className="px-2.5 py-1 rounded-xl text-xs font-semibold bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 transition-all active:scale-95"
                        >
                          Edit
                        </button>
                        <button
                          onClick={() => handleDelete(u._id)}
                          disabled={deletingId === u._id}
                          className="px-2.5 py-1 rounded-xl text-xs font-semibold bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 transition-all active:scale-95 disabled:opacity-40"
                        >
                          {deletingId === u._id ? "…" : "Delete"}
                        </button>
                      </div>
        ...
        >
          <div className="space-y-5">
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
              ...
              <div>
                <p className="text-sm font-bold text-slate-900">{editModal.name}</p>
                <p className="text-xs text-slate-600">{editModal.email}</p>
                <p className="text-xs text-slate-500 mt-0.5">
                  Joined {editModal.joined} · {editModal.orderCount} orders
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["user", "admin", "deliveryBoy"] as Role[]).map((r) => (
                  <button
                    ...
                    onClick={() => setEditRole(r)}
                    className={`py-2.5 rounded-xl text-xs font-bold border transition-all ${editRole === r
                      ? roleColors[r] + " ring-2 ring-indigo-600/10"
            ...
            <div className="flex gap-3 pt-1">
              <button
                onClick={handleSaveEdit}
                disabled={saving}
                className="flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold transition-all active:scale-95 disabled:opacity-50"
              >
                {saving ? "Saving…" : "Save Changes"}
              </button>
              <button
                onClick={() => setEditModal(null)}
                className="flex-1 py-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 text-sm font-semibold transition-all"
              >
                Cancel
              </button>
            </div>
