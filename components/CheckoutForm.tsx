...
// components/CheckoutForm.tsx
// CHANGELOG: Auto-fills shipping form from session user data on mount
...
export default function CheckoutForm({ cart, subtotal, clientSecret, deliveryCharge }: Props) {
  const dispatch = useDispatch();
  const router = useRouter();
  const stripe = useStripe();
  const elements = useElements();
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      fullName: prev.fullName || user.name || "",
      phone: prev.phone || user.mobile || "",
      // address, city, postalCode, country left blank
      // (these aren't in the user profile yet)
    }));
  }, [session]);
  // ──────────────────────────────────────────────────────────
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.error || "Failed to place order.");
        if (paymentMethod === "stripe") {
          toast("Your card was not charged.", { icon: "ℹ️" });
        }
      }
    } catch {
      toast.error("Something went wrong. Please try again.");
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-bold text-slate-800">Shipping Address</h2>
            {/* Show auto-filled badge if session exists */}
            {session?.user && (
              <span className="text-xs text-emerald-600 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-full font-semibold">
          ...
          {!session?.user && (
            <div className="mt-4 flex items-center gap-3 px-4 py-3 rounded-xl bg-indigo-50 border border-indigo-100">
              ...
              <p className="text-xs text-indigo-600">
                ...
                to auto-fill your name and phone number.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
            ...
            <button
              onClick={() => setPaymentMethod("stripe")}
              className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all text-left ${paymentMethod === "stripe"
                  ? "border-indigo-500 bg-indigo-50"
                  : "border-slate-200 hover:border-slate-300"
                }`}
            >
